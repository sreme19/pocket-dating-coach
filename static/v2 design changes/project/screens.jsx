...
/* ============================================================
   5. CHAT
   ============================================================ */
function ChatScreen({ profile, onBack, pov = "man", aiBestieOn = true }) {
  const isWoman = pov === "woman";

  // The "other person" in this chat depends on POV.
  // pov=man: profile is Sarah (default discovery match).
  // pov=woman: the "other person" becomes Alex (SELF).
  const other = isWoman
    ? {
        id: "alex",
        name: window.SELF.firstName,
        age: window.SELF.age,
        photo: window.SELF.avatar,
        trustScore: window.SELF.trustScore,
        archEmoji: "\ud83c\udfaf",
        archetype: "casual_man"
      }
    : profile;

  // Visual side of each bubble: when POV flips, "me" and "them" swap.
  const visualFrom = (m) => isWoman ? (m.from === "me" ? "them" : "me") : m.from;

  const myArchEmoji   = isWoman ? "\ud83d\udc8e" : "\ud83c\udfaf";
  const myArchLabel   = isWoman ? "Spoilt Woman" : "Casual Man";
  const theirArchEmoji= other.archEmoji;
  const theirArchLabel= isWoman ? "Casual Man" : "Spoilt Woman";
  const myTrust       = isWoman ? profile.trustScore : window.SELF.trustScore;
  const theirTrust    = other.trustScore;

  const [messages, setMessages] = useStateS(window.MESSAGES);
  const [draft, setDraft] = useStateS("");
  const [typing, setTyping] = useStateS(false);
  const [aiOn, setAiOn] = useStateS(aiBestieOn);
  const [aiConfig, setAiConfig] = useStateS(window.AI_BESTIE_DEFAULT);
  const [showSetup, setShowSetup] = useStateS(false);
  const threadRef = useRefS(null);

  // Keep local AI state in sync with the tweak when it flips externally.
  useEffectS(() => { setAiOn(aiBestieOn); }, [aiBestieOn]);
  ...
  const send = () => {
    ...
    setMessages(m => [...m, { id: nextId, from: isWoman ? "them" : "me", text: txt, time: "now" }]);
    setDraft("");
    // simulated reply
  ...
  // The bestie displayed: in man POV use HER per-profile bestie; in woman
  // POV the user IS Sarah-equivalent so use her own config.
  const herBestie = profile?.bestie || { name: "Mira", passed: 1, total: 5 };
  const bestieDisplayName = isWoman ? aiConfig.name : herBestie.name;

  const renderBubble = (m) => {
    const vf = visualFrom(m);
    if (m.fromAI) {
      return (
        <div key={m.id} className={`bubble-ai-wrap ${vf}`}>
          <div className={`bubble ${vf} from-ai`}>{m.text}</div>
          <AIBestieBubbleBadge name={bestieDisplayName} />
        </div>
      );
    }
    return <div key={m.id} className={`bubble ${vf}`}>{m.text}</div>;
  };
  ...
  return (
    <div className="screen enter chat" data-screen-label="05 Chat">
      {showSetup && (
        <AIBestieSetup
          initial={aiConfig}
          onSave={(c) => { setAiConfig(c); setAiOn(true); setShowSetup(false); }}
      ...
      <div className="chat-header">
        ...
        <div className="avatar" style={{ backgroundImage: `url(${other.photo})` }} />
        <div className="who">
          <span className="name">{other.name}, {other.age}</span>
          <span className="sub"><span className="dot" /> Verified · trust {theirTrust}</span>
        </div>
        <span className="spacer" />
      ...
      <div className="intent-banner">
        <div className="pair">
          <em>{myArchEmoji}</em> {myArchLabel} <span style={{ color: "var(--text-3)" }}>matched with</span> <em>{theirArchEmoji}</em> {theirArchLabel}
        </div>
        <div className="sub">
          <span>Both verified</span>
          <span>·</span>
          <span>trust <span className="score">{myTrust}</span> & <span className="score">{theirTrust}</span></span>
      ...
      {!isWoman && aiOn && (
        <AIBestieIncomingBanner herName={profile.name} bestie={herBestie} />
      )}

      <div className="thread" ref={threadRef}>
        <div className="daydiv">Yesterday</div>
        {messages.slice(0, 3).map(renderBubble)}
        <div className="daydiv">Today</div>
        {messages.slice(3).map(renderBubble)}
      ...
      {isWoman
        ? (aiOn
            ? <AIBestieActiveCard
                config={aiConfig}
                onTakeOver={() => setAiOn(false)}
                onTweak={() => setShowSetup(true)}
                onAddProbe={(p) => console.log("Probe added:", p)}
              />
            : <AIBestieEnablePrompt onConfigure={() => setShowSetup(true)} />)
        : <AIBestieLocked unlocksIn={window.AI_BESTIE_LOCK.unlocksIn} reason={window.AI_BESTIE_LOCK.reason} />}

      <div className="chat-input">
        <button className="icon-btn" title="Photo"><Icon name="camera" size={18} /></button>
        <div className="input-shell">
          <input
            placeholder={isWoman && aiOn ? `${aiConfig.name} is on duty — chime in if you want` : "Type something honest…"}
            value={draft}
