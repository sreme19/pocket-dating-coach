#!/usr/bin/env tsx
/**
 * Apply the beta-invites migration (20260715000000_create_beta_invites.sql):
 * verified_vibe_referral_links + verified_vibe_beta_signups. Idempotent —
 * "already exists" errors are tolerated so it can be re-run safely.
 *
 * Usage: tsx --env-file=.env.local scripts/apply-beta-invites.ts
 */
...
const MIGRATION = 'supabase/migrations/20260715000000_create_beta_invites.sql';
...
async function main() {
  ...
  console.log(`\n── Tables ─────────────────────────────`);
  console.log(`verified_vibe_referral_links : ${links ?? 0} rows`);
  console.log(`verified_vibe_beta_signups   : ${signups ?? 0} rows`);
