-- attention_messages
-- Stores "Secret Admirer" (female→male) and "Craving Attention" (male→female)
-- pre-match messages from the discover feed.
-- One message per (sender, recipient) pair — enforced by unique constraint.
-- Recipient can reply once via the reply_content column.
