...
DROP POLICY IF EXISTS "Users can insert own artifacts" ON user_artifacts;
CREATE POLICY "Users can insert own artifacts"
ON user_artifacts FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can read own artifacts" ON user_artifacts;
CREATE POLICY "Users can read own artifacts"
ON user_artifacts FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "service_role_all_artifacts" ON user_artifacts;
