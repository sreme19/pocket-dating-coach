-- Refer & Earn Flow 2 (women invite women) + Flow 3 (men invite women):
-- CASH referral ledger + signup mood.
...
-- Earned only when the invited WOMAN completes verification (liveness + photos),
-- never on mere signup.

-- (a) Mood/intent the referrer chose, carried via /beta/<token>?m=<mood> and
--     stored on the invitee's signup. STORAGE ONLY.
...
'Refer & Earn cash-payout ledger (Flows 2+3). One row per verified woman referral. Tiers: #1-25 = 100 INR, #26-100 = 150 INR, cap 100. UNIQUE(referred_user_id) enforces idempotency + cap. Admin marks paid; no automated transfer.';;
