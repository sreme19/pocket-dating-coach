<script lang="ts">
  ...
  const questions: Question[] = [
    ...
    {
      id: 'boundaries_discretion',
      type: 'multi-select',
      question: '🛡️ What relationship standards matter most to you?',
      subtitle: 'Select all that apply',
      note: 'This helps us match you with partners who share your standards.',
      options: [
        { value: 'discretion_matters',         label: '🔒 Discretion matters' },
        ...
        { value: 'safety_trust_first',         label: '🛡 Safety & trust first' },
  ...
  {#if step === 'questions'}
    <div class="questions-container" transition:fade={{ duration: 200 }}>
      {#each [current()] as q (q.id)}
        <div class="question-card" transition:scale={{ duration: 200 }}>
          ...
          {#if q.optional && q.type === 'multiple-choice'}
            <p class="optional-note">Optional — you can skip this question.</p>
          {/if}
        </div>
      {/each}
    </div>
    ...
    <div class="button-group">
      ...
      <button
        ...
        onclick={handleNext}
        disabled={loading || (!current().optional && !isAnswered(current()))}
      >
...
<style>
  .casual-generous-preferences-step {
    display: flex;
