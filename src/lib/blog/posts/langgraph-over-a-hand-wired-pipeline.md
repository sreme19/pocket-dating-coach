...
date: 2026-08-27
summary: A pipeline of agents is a plain for-loop until two of them have to run at once and write to the same state object, and then the loop either serialises work that never needed serialising or lets the two writes race. I wired three decision engines this way instead — a Game of Thrones strategy oracle, an intelligence-operations oracle, and a survey-analysis pipeline — each a state machine of eight to ten nodes over one shared typed state, with the parallel branches fanning out and converging on a single node, a one-line merge rule letting both write the same list without a collision, and a validation gate that halts the run before the expensive model step. What it does not buy you is a reason to reach for any of this when your pipeline is honestly a straight line.
