<script lang="ts">
	...
	let mobileScreens  = $derived(new Set(data.mobileErrors.map((e) => e.screen)).size);
	let mobileErrTypes_ = $derived(new Set(data.mobileErrors.map((e) => e.error_type)).size);
</script>
...
<div class="min-h-screen bg-[#060d1a] p-6 text-slate-100">
	...
	<div class="mb-6 flex items-center justify-between">
		...
		<form
			...
			use:enhance={() => {
				running = true;
				return async ({ update }) => { await update(); running = false; };
			}}
		>
			<button
				type="submit"
		...
		{#if serverTotalPages > 1}
			<div class="mt-4 flex items-center justify-center gap-2">
				<button onclick={() => serverPage = Math.max(1, serverPage_ - 1)} disabled={serverPage_ === 1}
					class="rounded-lg border border-white/[0.1] bg-white/[0.03] px-3 py-1.5 text-xs text-slate-400 disabled:opacity-30 hover:bg-white/[0.06]">
					← Prev
				</button>
				{#each Array.from({length: serverTotalPages}, (_, i) => i + 1) as p}
					{#if Math.abs(p - serverPage_) <= 2 || p === 1 || p === serverTotalPages}
						<button onclick={() => serverPage = p}
							class="rounded-lg border px-3 py-1.5 text-xs {p === serverPage_ ? 'border-emerald-500/50 bg-emerald-500/10 text-emerald-400' : 'border-white/[0.1] bg-white/[0.03] text-slate-400 hover:bg-white/[0.06]'}">
							{p}
						</button>
	...
	{#if activeSection === 'mobile'}
		...
		<div class="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
			...
			<div class="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
				<div class="text-2xl font-bold text-orange-400">{mobileScreens}</div>
				<div class="mt-1 text-xs text-slate-400">Screens affected</div>
			</div>
			<div class="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
				<div class="text-2xl font-bold text-purple-400">{mobileErrTypes_}</div>
				<div class="mt-1 text-xs text-slate-400">Error types</div>
			</div>
			<div class="rounded-xl border border-white/[0.06] bg-white/[0.03] p-4">
				<div class="text-2xl font-bold text-slate-300">{new Set(data.mobileErrors.map(e => e.user_id).filter(Boolean)).size}</div>
		...
		<div class="mb-4 flex flex-wrap items-center gap-2">
			<select bind:value={mobileScreenFilter} class="rounded-lg border border-white/[0.1] bg-white/[0.04] px-3 py-1.5 text-sm text-slate-300 outline-none focus:border-orange-500/50">
				{#each mobileScreenNames as s}
					<option value={s}>{s === 'all' ? 'All screens' : s}</option>
				{/each}
			</select>
			<select bind:value={mobileErrTypeFilter} class="rounded-lg border border-white/[0.1] bg-white/[0.04] px-3 py-1.5 text-sm text-slate-300 outline-none focus:border-orange-500/50">
				{#each mobileErrTypes as t}
					<option value={t}>{t === 'all' ? 'All error types' : t}</option>
				{/each}
			</select>
				...
				<tbody>
					{#each mobilePaged as e}
						<tr class="border-b border-white/[0.04] bg-red-500/[0.03] hover:bg-white/[0.02]">
							...
							<td class="px-4 py-2.5">
								{#if e.action}
									<span class="font-mono text-xs text-slate-400">{e.action}</span>
								{:else}
									<span class="text-slate-600">—</span>
								{/if}
							</td>
							...
							<td class="px-4 py-2.5 text-xs">
								{#if e.error_message}
									<span class="text-red-400">{e.error_message.slice(0, 120)}{e.error_message.length > 120 ? '…' : ''}</span>
									{#if e.metadata?.stack}
										<details class="mt-1">
											<summary class="cursor-pointer text-slate-500 hover:text-slate-300">stack trace</summary>
											<pre class="mt-1 max-h-32 overflow-auto rounded bg-white/[0.04] p-2 text-xs text-slate-400 whitespace-pre-wrap">{e.metadata.stack}</pre>
										</details>
									{/if}
								{:else}
									<span class="text-slate-600">—</span>
							...
							<td class="px-4 py-2.5">
								{#if e.user_id}
									<span class="font-mono text-xs text-slate-500">{e.user_id.slice(0, 8)}…</span>
								{:else}
				...
				{#each Array.from({length: mobileTotalPages}, (_, i) => i + 1) as p}
					{#if Math.abs(p - mobilePage_) <= 2 || p === 1 || p === mobileTotalPages}
						<button onclick={() => mobilePage = p}
							class="rounded-lg border px-3 py-1.5 text-xs {p === mobilePage_ ? 'border-orange-500/50 bg-orange-500/10 text-orange-400' : 'border-white/[0.1] bg-white/[0.03] text-slate-400 hover:bg-white/[0.06]'}">
							{p}
						</button>
