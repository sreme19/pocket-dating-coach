 ...
 * 
 * Deactivates AI Bestie for a specific match conversation.
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	try {
		...
		logError('POST /api/ai-bestie/deactivate', err, ErrorType.VALIDATION_ERROR);
	...
	try {
		...
		await deactivateAssistant(userId, matchId, 'bestie');

		// Log successful deactivation
		console.log(`[AI Bestie] Deactivated for user ${userId}, match ${matchId}`);
		...
		logError('POST /api/ai-bestie/deactivate', err, ErrorType.DATABASE_ERROR, {
			userId,
			matchId
		});
