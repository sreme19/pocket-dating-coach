...
/**
 * GET /api/preferences
 * Retrieve user's preferences.md profile
 * 
...
export const GET: RequestHandler = async ({ locals }) => {
	...
	try {
		...
		logError('GET /api/preferences', err, ErrorType.DATABASE_ERROR, {
			userId
		});
 ...
 * Update user's preferences.md profile with version tracking
 * 
 * Request body:
 * {
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	try {
		...
		logError('POST /api/preferences', err, ErrorType.VALIDATION_ERROR);
	...
	if ('emotionalSignals' in body.updates) typeMap['emotionalSignals'] = 'object'; // array is typeof 'object'
	...
	try {
		...
		logError('POST /api/preferences', err, ErrorType.DATABASE_ERROR, {
			userId,
			reason
		});

		throwDatabaseError('POST /api/preferences', err, 'Failed to update preferences profile');
	}
};
