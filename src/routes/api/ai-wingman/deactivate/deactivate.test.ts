...
/**
 * Unit tests for POST /api/ai-wingman/deactivate endpoint
 * 
...
describe('POST /api/ai-wingman/deactivate', () => {
	...
	describe('Deactivation Logic', () => {
		...
		it('should call deactivateAssistant with correct parameters for wingman', async () => {
			// Arrange
			const userId = 'test-user-123';
	...
	describe('Success Response', () => {
		...
		it('should log successful deactivation', async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith(
				'[AI Wingman] Deactivated for user test-user-123, match match-123'
			);
	...
	describe('Error Handling', () => {
		...
		it('should handle database errors from deactivateAssistant', async () => {
			...
			await expect(
				POST({ request: mockRequest, locals: mockLocals } as any)
			).rejects.toThrow('Failed to deactivate AI Wingman. Please try again.');
