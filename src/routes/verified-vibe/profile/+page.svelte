<script lang="ts">
  ...
  async function generateMoreInsights(category: 'wealth' | 'linkedin' | 'lifestyle') {
    ...
    try {
      ...
      const res = await fetch('/api/verified-vibe/wingman', {
        ...
        body: JSON.stringify({
          message: `Suggest 3 more insight chips from my ${isWealth ? 'wealth / financial' : isLifestyle ? 'lifestyle / travel / experience' : 'LinkedIn / career'} documents. Existing labels to avoid: ${existingLabels.join(', ')}. Aggregated data: ${aggregated}`,
  ...
  function commitPendingInsights(category: 'wealth' | 'linkedin' | 'lifestyle') {
