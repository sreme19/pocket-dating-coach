<script lang="ts">
  ...
  async function generateMoreInsights(category: 'wealth' | 'linkedin') {
    ...
    try {
      ...
      const res = await fetch('/api/verified-vibe/wingman', {
        ...
        body: JSON.stringify({
          message: `Suggest 3 more insight chips from my ${isWealth ? 'wealth / financial' : 'LinkedIn / career'} documents. Existing labels to avoid: ${existingLabels.join(', ')}. Aggregated data: ${aggregated}`,
  ...
  function commitPendingInsights(category: 'wealth' | 'linkedin') {
    const pending = category === 'wealth' ? pendingWealthInsights : pendingCareerInsights;
